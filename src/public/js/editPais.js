document.addEventListener("DOMContentLoaded", () => {
  const form = document.getElementById("edit-pais-form");
  ...
  form.addEventListener("submit", async (event) => {
    ...
    const datosACargar = {
      ...
      capital: primeraLetraMayuscula(data.nombreCapital)
        ...
        : [],
      borders:
        typeof data.fronteras === "string"
          ? data.fronteras
              .split(",")
              .map((frontera) => frontera.trim().toUpperCase())
          : [],
    ...
    console.log("datos a guardar", datosACargar);

    try {
      // Realiza la solicitud POST al backend
      const response = await fetch(`/api/paises/editar/${id}`, {
        method: "PUT",
        headers: {
      ...
      if (response.ok) {
        ...
        Swal.fire({
          ...
          position: "top-end",
          icon: "success",
          title: "País modificado con éxito",
        ...
        console.log("Hubo un problema al editar el país.");
        // Mostrar alerta de problema
        Swal.fire({
          title: "Hubo un problema al editar el país",
